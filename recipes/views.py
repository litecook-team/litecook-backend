...
from datetime import date
from .models import Recipe, RecipeOfDay
...
class AIChatView(APIView):
    ...
    def post(self, request):
        ...
        # === ОТРИМУЄМО РЕАЛЬНИЙ РЕЦЕПТ ДНЯ ===
        try:
            today = date.today()
            daily_record = RecipeOfDay.objects.filter(date=today).first()
            if daily_record:
                recipe_of_day_text = f"Сьогоднішній 'Рецепт дня' — це [{daily_record.recipe.title}](/recipe/{daily_record.recipe.id})"
            else:
                recipe_of_day_text = "Сьогодні 'Рецепт дня' ще не обрано."
        except Exception:
            recipe_of_day_text = ""

        # === ПОВЕРНУТО БЛОК НАВІГАЦІЇ ТА ДОДАНО ОБРОБКУ НЕІСНУЮЧИХ СТОРІНОК ===
...
1. ГОЛОВНА СТОРІНКА (/): Знайомить з користю проєкту (сезонний пошук, легке планування раціону). {recipe_of_day_text}.
2. ТИЖНЕВЕ МЕНЮ (/menu): На цій сторінці користувач СТВОРЮЄ СВІЙ ПЛАН ХАРЧУВАННЯ (додає рецепти на кожен день). НАЙГОЛОВНІШЕ: тут Є ФУНКЦІЯ АВТОМАТИЧНОЇ ГЕНЕРАЦІЇ СПИСКУ ПРОДУКТІВ! Користувач може завантажити готовий PDF-список покупок для обраних рецептів або надіслати його на email. Якщо користувач питає, як згенерувати список продуктів — завжди кажи, що ця функція є, і направляй його на сторінку [Тижневе меню](/menu).
3. МІЙ ПРОФІЛЬ (/profile): Тут знаходиться віртуальний холодильник ("Мої продукти"), статистика улюблених рецептів, налаштування алергій та дієт.
4. ПІДІБРАТИ РЕЦЕПТ (/recipes): Доступні 10 фільтрів для пошуку ідеальної страви.
...
5. ПОСИЛАННЯ ТА ФОРМАТУВАННЯ: КАТЕГОРИЧНО ЗАБОРОНЕНО ховати посилання під словом "тут" або "сюди". Завжди роби клікабельною саму назву сторінки! Роби прості марковані списки. Назви кнопок та галочок виділяй жирним шрифтом. ПРАВИЛЬНО: "Сплануйте свій раціон у розділі [Тижневе меню](/menu).". НЕПРАВИЛЬНО: "Сплануйте раціон [тут](/menu).".
